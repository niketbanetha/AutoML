...
def regression_func(dftrain=None,dftest=None,x=None,y=None):
    ...
    if s=="DecisionTreeRegressor":
        tpred=r.preddt
    elif s=="Poly_Degree_2":
        tpred=r.predd2
    elif s=="RandomForestRegressor":
        tpred=r.predrfg
    elif s=="LinearRegression":
        tpred=r.predlm
...
#     except:
#         st.error("Pickle File cannot be developed.")
        ...
        if checker(dftrain):
            if g=="Regression":
                if dftrain.empty==False and colname!="":
                    ...
                    show=dftrain[y].head(10)
                    show["Predicted Value"]=d[1][:10]
                    st.table(show)
                    if len(d)==3:
                        st.markdown(download(d[2]), unsafe_allow_html=True)
            else:
                    if dftrain.empty==False and colname!="":
                        x=list(dftrain.columns)
                        x.remove(colname)
